  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-levant-black text-white font-sans selection:bg-levant-gold selection:text-black">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-levant-gray bg-levant-black/95 sticky top-0 z-50">
        ...
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-levant-gold p-2">
      ...
      {/* Sidebar Navigation */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-72 bg-levant-dark border-r border-levant-gray transform transition-transform duration-300 ease-in-out
        md:sticky md:top-0 md:h-screen md:translate-x-0
        ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-10 text-center border-b border-levant-gray/50">
          <h1 className="text-4xl font-serif text-levant-gold tracking-widest mb-2 font-bold">LEVANT</h1>
          <p className="text-xs text-neutral-400 uppercase tracking-widest">Personeel</p>
        </div>
        ...
        <nav className="p-6 space-y-4 mt-4">
          {navItems.map((item) => (
            <button
              ...
              className={`
                w-full flex items-center gap-4 px-6 py-5 rounded-2xl transition-all duration-200 text-lg
                ${activeTab === item.id 
      ...
      <main className="flex-1 relative min-h-screen bg-[url('https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center bg-fixed">
        ...
        <div className="relative z-10 p-4 md:p-10 max-w-7xl mx-auto flex flex-col min-h-screen">
          {children}
        </div>
      </main>
    </div>
